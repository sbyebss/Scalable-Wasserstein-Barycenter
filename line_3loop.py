...
"""""""""""""""""""""""""""""""""""""""""""""""""""
train_data = g_data.marginal_data_line_3loop_ficnn(cfg)
train_loader = torch.utils.data.DataLoader(
    train_data, batch_size=cfg.BATCH_SIZE, shuffle=False, **kwargs)

# PLU.plot_2dmarginal(cfg, train_data, results_save_path, -3, 3)
...
def train(epoch):
    ...
    for batch_idx, real_data in enumerate(train_loader):
        ...
        if cfg.INPUT_DIM > 1:
            ...
            epsilon = real_data[:, 0, cfg.NUM_DISTRIBUTION].reshape(-1, 1)
...
for epoch in range(1, cfg.epochs + 1):
    ...
    if epoch % 1 == 0:
        LLU.dump_nn(generator_h, convex_f, convex_g, epoch,
                    model_save_path, num_distribution=10)

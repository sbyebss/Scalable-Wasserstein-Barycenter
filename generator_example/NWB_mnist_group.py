...
#                 f'/{cfg.load_epoch}_h_{cfg.repeat}.png', gan=True)

#! g

miu_list, marginal_list = CDR.barycenter_pushforward(
    cfg, PTU.device, results_save_path=results_save_path, load_epoch=cfg.load_epoch, type_data=cfg.type_data, return_marginal=True)

# for idx in range(cfg.NUM_DISTRIBUTION):
# PLU.mnist_alone(miu_list[idx], results_save_path +
#                 f'/{cfg.load_epoch}_g{idx}.png', gan=True)
# PLU.mnist_alone(marginal_list[idx], results_save_path +
#                 f'/{cfg.load_epoch}_m{idx}.png', gan=True, range_sample=(-1, 1))

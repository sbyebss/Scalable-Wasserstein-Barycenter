...
from optimal_transport_modules.cfg import CfgLine as Cfg_class
...
if __name__ == "__main__":
    ...
    tmp_epoch = 23
    for idx in range(20):
        PLU.draw_marginal_lines('input_data/line/components.txt')
        miu = CDR.barycenter_sampler(
            cfg, PTU.device, results_save_path=results_save_path, load_epoch=tmp_epoch)
        PLU.sns_scatter_handle(
            miu.cpu().detach().numpy(), -3, 3,
            results_save_path + f'/{tmp_epoch}_{cfg.N_TEST}_{cfg.opacity}_{cfg.scatter_size}_{idx}.png', scatter_size=cfg.scatter_size, new_fig=False)

    miu_list = CDR.barycenter_pushforward(
        cfg, PTU.device, results_save_path=results_save_path, load_epoch=tmp_epoch, type_data='line')

    for idx in range(cfg.NUM_DISTRIBUTION):
        PLU.draw_marginal_lines('input_data/line/components.txt')
        PLU.sns_scatter_handle(
            miu_list[idx].cpu().detach().numpy(), -3, 3,
            results_save_path + f'/{tmp_epoch}_g{idx}_{cfg.N_TEST}_{cfg.opacity}_{cfg.scatter_size}.png', scatter_size=cfg.scatter_size, new_fig=False)
